...
?>
<?php 
if (isset($_GET['disapprove-id'])) {
    $cid = $_GET['disapprove-id'];
    $status = 0;
    $update = "UPDATE blogs SET status = '$status' WHERE blog_id = '$cid'";
    if (mysqli_query($con, $update)) {
        echo "<script>alert('Blog disapprove, status set to 0');</script>";
    } else {
        echo "<script>alert('Failed to disapprove, please try again');</script>";
    }
}
?>
                ...
                <tr>
                    ...
                    <td>
                        <?php if ($status == NULL || $status == 0) { ?>
                            <p><a href="manage-blogs.php?approve-id=<?php echo $row['blog_id']; ?>">Enable</a></p>
                        <?php } else { ?>
                            <p><a href="manage-blogs.php?disapprove-id=<?php echo $row['blog_id']; ?>">Disable</a></p>
                        <?php } ?>
                    </td>
                </tr>
            <?php
